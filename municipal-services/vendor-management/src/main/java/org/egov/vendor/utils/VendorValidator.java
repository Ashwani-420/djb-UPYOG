...
public class VendorValidator {
    ...
    private void validateWorkOrderRequiredFields(VendorWorkOrder workOrder) {
        ...
        if (StringUtils.isEmpty(workOrder.getVendorId())) {
        ...
        // Fix: Use StringUtils.isEmpty() for String
        if (StringUtils.isEmpty(workOrder.getMobileNumber())) {
            throw new CustomException("EG_VWO_MOBILE_REQUIRED", 
                "Mobile number is required");
        }
        
        // Fix: Use StringUtils.isEmpty() for String
        if (StringUtils.isEmpty(workOrder.getEmailId())) {
            throw new CustomException("EG_VWO_EMAIL_REQUIRED", 
                "Email is required");
        }
        
        if (StringUtils.isEmpty(workOrder.getServiceType())) {
    ...
    private void validateWorkOrderDataFormats(VendorWorkOrder workOrder) {
        ...
        if (!StringUtils.isEmpty(workOrder.getAlternateNumber())) {
    ...
    private void validateWorkOrderBusinessRules(VendorWorkOrder workOrder) {
        ...
        if (duration > maxDuration) {
            throw new CustomException("EG_VWO_DURATION_TOO_LONG", 
                "Work order duration cannot exceed 1 year");
        }
